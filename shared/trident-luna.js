...
(function () {
  ...
  function layout(m) {
    ...
    if (m.mode === "trident") {
      ...
      m.dots = [];
      var LN = Math.round(Math.min(108, Math.max(63, m.W * 0.22)));
      for (var j = 0; j < LN; j++) { var ring = j % 3; m.dots.push({ a: j * 2.39996, ring: ring, rr: m.rings[ring], ph: rnd(0, 6.28), size: rnd(0.9, 1.9) }); }
    }
  }
